...
export function Ic({ n, s = 18, className = "" }: { n: string; s?: number; className?: string }) {
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
      {P[n] ?? P.info}
    </svg>
  );
}
...
export function LevelChip({ score, clave = false, sm = false }: { score: number | null; clave?: boolean; sm?: boolean }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-md font-bold mono ${sm ? "px-1.5 py-0.5 text-[10px]" : "px-2 py-0.5 text-[11px]"} ${n.cls}`} title={score === null ? "Sin datos" : score.toLocaleString("es-ES", { maximumFractionDigits: 1 })}>
...
export function EstadoBadge({ estado }: { estado: string }) {
  const map: Record<string, string> = {
    "Borrador": "bg-ambl text-amb", "En revisión": "bg-azul text-azu", "Finalizada": "bg-virl text-vird",
  };
  const dot: Record<string, string> = { "Borrador": "#c98a12", "En revisión": "#2c6e8f", "Finalizada": "#0e7c66" };
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[11px] font-bold ${map[estado] ?? "bg-line/60 text-ink2"}`}>
      <span className="h-1.5 w-1.5 rounded-full" style={{ background: dot[estado] ?? "#7c929b" }} />
      {estado}
    </span>
  );
}
...
export function SelChip({ active, onClick, children, color }: { active: boolean; onClick: () => void; children: ReactNode; color?: string }) {
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-[12px] font-semibold transition-all duration-200 cursor-pointer ${
        active ? "border-transparent text-white shadow-sm scale-[1.03]" : "border-line2 bg-card text-ink2 hover:border-ink3 hover:text-ink"
      }`}
      style={active ? { background: color ?? "#0e7c66" } : undefined}
    >
      {active && <Ic n="check" s={12} />}
      {children}
    </button>
  );
}
...
export function Bar({ value, color = "#0e7c66", className = "", h = 6 }: { value: number; color?: string; className?: string; h?: number }) {
  ...
  return (
    <div ref={ref} className={`w-full overflow-hidden rounded-full bg-line/70 ${className}`} style={{ height: h }}>
      <div
        className="h-full rounded-full transition-[width] duration-1000 ease-out"
        style={{ width: inView ? `${Math.max(0, Math.min(100, value))}%` : "0%", background: color }}
      />
    </div>
  );
}
...
export function Sparkline({ points, color = "#0e7c66", w = 120, h = 34 }: { points: (number | null)[]; color?: string; w?: number; h?: number }) {
  ...
  const pts = points.map((p, i) => {
    ...
    const y = h - 4 - ((p - min) / (max - min)) * (h - 8);
    return `${x},${y}`;
  }).filter(Boolean);
  const d = pts.length ? `M${pts.join(" L")}` : "";
  return (
    <svg width={w} height={h} className="overflow-visible">
      <path d={`${d} L${w - 3},${h} L3,${h} Z`} fill={color} opacity="0.1" />
      <path d={d} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" className="draw-path" />
      {pts.map((p, i) => { const [x, y] = p!.split(",").map(Number); return <circle key={i} cx={x} cy={y} r="2.4" fill={color} />; })}
    </svg>
  );
}
...
export function Modal({ open, onClose, title, children, wide = false }: { open: boolean; onClose: () => void; title: string; children: ReactNode; wide?: boolean }) {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-3 sm:p-6" role="dialog" aria-modal="true">
      ...
      <div className={`pop relative w-full ${wide ? "max-w-3xl" : "max-w-xl"} max-h-[88vh] overflow-y-auto rounded-xl border border-line bg-panel shadow-2xl`}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-line bg-panel/95 px-5 py-3.5 backdrop-blur">
          ...
          <button onClick={onClose} className="rounded-md p-1.5 text-ink2 transition hover:bg-line/60 hover:text-ink cursor-pointer" aria-label="Cerrar">
            <Ic n="x" s={18} />
          </button>
        </div>
...
export function SectionHead({ kicker, title, desc, actions }: { kicker: string; title: string; desc?: string; actions?: ReactNode }) {
  const { ref, inView } = useInView<HTMLDivElement>(0.3);
  return (
    <div ref={ref} className={`rv ${inView ? "in" : ""} mb-6 flex flex-wrap items-end justify-between gap-4`}>
      <div className="max-w-2xl">
        <p className="mono mb-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-vir">▞ {kicker}</p>
        <h1 className="lm font-display text-[26px] leading-tight font-extrabold tracking-tight text-ink sm:text-[32px]">
          <span>{title}</span>
        </h1>
...
export function EmptyState({ icon = "info", title, desc, action }: { icon?: string; title: string; desc?: string; action?: ReactNode }) {
  return (
    <div className="card flex flex-col items-center justify-center gap-2 px-8 py-14 text-center">
...
export function ToastHost() {
  const { toast } = useApp();
  const [show, setShow] = useState<Toast | null>(null);
  ...
  if (!show) return null;
  return (
    <div key={show.key} className="toast-in fixed bottom-5 right-5 z-[90] flex items-center gap-2.5 rounded-xl border border-vir/30 bg-night px-4 py-3 text-[13px] font-semibold text-paper shadow-2xl no-print">
      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-vir text-white"><Ic n="check" s={13} /></span>
      {show.msg}
    </div>
  );
}
