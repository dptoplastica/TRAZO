...
interface LayoutProps { children: ReactNode; currentUser: { id: string; nombre: string; rol: string; email: string }; onLogout: () => void; }
...
export function Layout({ children, currentUser, onLogout }: LayoutProps) {
    ...
    <aside className="relative flex h-full w-[248px] flex-col overflow-hidden bg-night text-paper">
      ...
      <nav className="relative z-10 flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        {items.map((n) => {
          ...
          return (
            <button key={n.id} onClick={() => { nav(n.id); setOpen(false); }} className={`group relative flex w-full cursor-pointer items-center gap-3 rounded-lg px-3 py-[9px] text-left text-[13px] font-semibold transition-all duration-200 ${active ? "bg-night3 text-white" : "text-mist hover:bg-night2 hover:text-white"}`}>
      ...
      <div className="relative z-10 border-t border-night3 px-4 py-4">
        ...
        <select value={d.teacherId} onChange={(e) => { const t = d.teachers.find((x) => x.id === e.target.value); set((s) => ({ ...s, teacherId: e.target.value, role: t?.rol === "admin" ? "admin" : "profesor" })); }} className="inp mono mt-3 !border-night3 !bg-night2 !py-1.5 text-[11.5px] !text-mist focus:!border-vir" aria-label="Cambiar de usuario">
          {d.teachers.map((t) => (<option key={t.id} value={t.id}>{t.nombre} · {t.rol === "admin" ? "Jefatura" : "Profesor/a"}</option>))}
        </select>
      </div>
    </aside>
  );
  ...
  return (
    <div className="noise min-h-screen bg-paper bg-blueprint">
      ...
      {open && (<div className="no-print fixed inset-0 z-50 lg:hidden"><div className="absolute inset-0 bg-night/60" onClick={() => setOpen(false)} /><div className="absolute inset-y-0 left-0 shadow-2xl">{sidebar}</div></div>)}
          ...
          <div className="flex items-stretch">
            ...
            <div className="flex flex-1 items-center divide-x divide-line overflow-x-auto">
              ...
              <div className="hidden items-center gap-2 px-4 sm:flex">
                ...
                <span className="mono text-[10px] font-bold uppercase tracking-widest text-ink2">{isAdmin ? "Acceso · Departamento" : "Acceso · Profesorado"}</span>
                <button onClick={onLogout} className="ml-2 rounded-md border border-line px-2 py-1 text-[11px] font-semibold text-ink2 transition hover:border-verm hover:bg-verml hover:text-verm cursor-pointer" title="Cerrar sesión">Salir</button>
              </div>
            </div>
          </div>
        </header>
